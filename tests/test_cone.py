...
def test_volume_cone_valid_inputs():
    ...
    radius, height = 3.0, 3.0
    expected = 9 * math.pi
    assert volume_cone(radius, height) == expected

def test_volume_cone_negative_dimensions():
    """Document current behaviour when negative dimensions are used."""
...
def test_volume_cone_float_tolerance():
    ...
    expected = (1/3) * math.pi * radius**2 * height
